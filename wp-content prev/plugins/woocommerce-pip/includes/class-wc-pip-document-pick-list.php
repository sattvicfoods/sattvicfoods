	...
	private function get_category_table_rows() {
		...
		foreach ( $this->order_ids as $order_id ) {
			...
			if ( $this->order && $this->get_items_count() > 0 ) {
				...
				foreach ( $items as $item_id => $item ) {
					...
					// if meta does not exist, then just store the current product ID
					...
					foreach ( $formatted_meta as $meta_values ) {
						...
						$formatted_meta_key   = trim( $meta_values['key'] );
						$formatted_meta_value = trim( $meta_values['value'] );

						if ( ! empty( $cat_table_rows[ $key ][ $formatted_meta_key ][ $formatted_meta_value ]['orders'] ) ) {
							$order_counter++;
						}

						$cat_table_rows[ $key ][ $formatted_meta_key ][ $formatted_meta_value ]['orders'][ $order_counter ] = $this->order;
						$cat_table_rows[ $key ][ $formatted_meta_key ][ $formatted_meta_value ]['item'][ $item_id ]         = $item;
						$cat_table_rows[ $key ][ $formatted_meta_key ][ $formatted_meta_value ]['product']                  = SV_WC_Plugin_Compatibility::is_wc_version_gte_3_0() ? $item->get_product() : $this->order->get_product_from_item( $item );
					}
				}
			}
		}
		...
		if ( ! empty( $cat_table_rows ) ) {
			...
			foreach ( $cat_table_rows as $term_name => $data ) {
				...
				foreach ( $data as $formatted_meta_data ) {

					foreach ( $formatted_meta_data as $item_data ) {
