...
class FUE_Addon_WooCommerce_Order_Importer {
    ...
    public function filter_orders( $data ) {
        ...
        return array( $email->id => $data['orders'] );
    }

    public function import_orders( $data, $limit = 100 ) {
        $imported   = array();
        $processed  = 0;
        $limit_reached = false;
        ...
        $remaining = $this->count_remaining_orders( $data );
        return array(
            'imported'          => $imported,
    ...
    private function filter_storewide_orders( $orders, $email ) {
        ...
        if ( $email->product_id && isset( $orders[ $email->id ] ) ) {
            $order_ids = implode(',', $orders[ $email->id ]);
            $orders = $wpdb->get_col( $wpdb->prepare(
                "SELECT order_id
        ...
        if ( !empty( $email->meta['excluded_categories'] ) ) {
            foreach ( $orders[ $email->id ] as $idx => $order_id ) {
                if ( in_array( $order_id, $email->meta['excluded_categories'] ) ) {
                    unset( $orders[ $email->id ][ $idx ] );
                }
            }

            // reset the indices
            $orders[ $email->id ] = array_values( $orders[ $email->id ] );
        }
    ...
    private function filter_customer_orders( $orders, $emails ) {
        return $orders;
    }
