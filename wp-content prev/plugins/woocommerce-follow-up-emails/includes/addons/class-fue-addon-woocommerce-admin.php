...
class FUE_Addon_Woocommerce_Admin {
    ...
    public function subscribers_settings() {
        include FUE_TEMPLATES_DIR .'/add-ons/settings-woocommerce-subscribers.php';
    }
    ...
    public function addon_save_subscribers_settings() {
        ...
        $enable = (isset($_POST['enable_checkout_subscription'])) ? (int)$_POST['enable_checkout_subscription'] : 0;
        $label  = $_POST['checkout_subscription_field_label'];

        update_option( 'fue_enable_checkout_subscription', $enable );
        update_option( 'fue_checkout_subscription_field_label', $label );
    }
    ...
    public function add_import_link_to_post_message( $messages ) {
        ...
        if ( $import ) {
            ...
            $messages[1] = __( 'Email saved. <a href="'. esc_url( $url ) .'" class="button">Import existing orders</a>', 'follow_up_emails' );
        }
