...
abstract class WC_PIP_Document {
	...
	protected function get_order_item_price_html( $item_id, $item ) {
		...
		// handle refunds
		if ( $this->has_refunds && absint( $this->order->get_total_refunded_for_item( $item_id ) ) > 0 ) {

			$refund_total = absint( $this->order->get_total_refunded_for_item( $item_id ) );
	...
	protected function get_order_item_meta( $item_id, $item, $product ) {
		...
		if ( SV_WC_Plugin_Compatibility::is_wc_version_gte_3_1() ) {
			...
			foreach ( $meta_data as $meta ) {
				...
				if ( $flat ) {
					$meta_list[] = wp_kses_post( $meta->display_key . ': ' . $meta->value );
				} else {
					$meta_list[] = '
