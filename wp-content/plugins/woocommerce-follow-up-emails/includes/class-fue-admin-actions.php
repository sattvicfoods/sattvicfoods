...
class FUE_Admin_Actions {
    ...
    static function update_settings() {
        ...
        if ( $section == 'crm' ) {
            ...
            if ( isset($_POST['bounce']) ) {
                update_option( 'fue_bounce_settings', $_POST['bounce'] );
            ...
            // process importing request
            $action = ( ! empty( $_POST['upload'] ) ) ? 'upload' : 'save';
            ...
            do_action( 'fue_settings_subscribers_save', $_POST );

        } elseif ( $section == 'auth' ) {
            $spf  = (isset($_POST['spf'])) ? $_POST['spf'] : array();
            $dkim = (isset($_POST['dkim'])) ? $_POST['dkim'] : array();

            update_option( 'fue_spf', $spf );
            update_option( 'fue_dkim', $dkim );

            do_action( 'fue_settings_authentication_save', $_POST );
        } else {
    ...
    static function manage_subscribers() {
        ...
        if ( !empty($post['button_add']) ) {
            ...
            exit;
        } elseif ( !empty($post['button_action']) ) {
            $action     = $post['action2'];
            $emails     = $post['email'];
            $email_ids  = '';
    ...
    public static function process_queue_delete_all() {
        ...
        $messages = array( sprintf( _n('%d email has been deleted', '%s emails have been deleted', $deleted, 'follow_up_emails'), $deleted ) );

        $message_nonce = wp_create_nonce( __FILE__ );
        set_transient( '_fue_messages_' . $message_nonce, array( 'messages' => $messages, 'error_messages' => array() ), 60 * 60 );
