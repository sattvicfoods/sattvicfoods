...
class FUE_Addon_Subscriptions extends FUE_Addon_Woocommerce_Scheduler {
            ...
            foreach ( $subscriptions as $user_id => $user_subscriptions ) {
                foreach ( $user_subscriptions as $sub_key => $subscription ) {
                    ...
                    if ( $subscription['product_id'] == $post['subscription_id'] || $subscription['variation_id'] == $post['subscription_id'] ) {
                        ...
                        $key = $user_id .'|'. $user_email .'|'. $first_name .' '. $last_name;
                        $recipients[$key] = array($user_id, $user_email, $first_name .' '. $last_name);
                    }
                }
            }
    ...
    public function email_variables_list( $email ) {
        ...
        ?>
        <li class="var hideable var_subscriptions"><strong>{item_name}</strong> <img class="help_tip" title="<?php _e('The subscription product\'s name', 'follow_up_emails'); ?>" src="<?php echo $woocommerce->plugin_url(); ?>/assets/images/help.png" width="16" height="16" /></li>
